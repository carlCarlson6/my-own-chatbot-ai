...
interface MessageComposerProps {
  ...
  sendStatus: 'idle' | 'sending' | 'streaming' | 'error'
}

export function MessageComposer({ onSend, sendStatus }: MessageComposerProps) {
  const [value, setValue] = useState('')
  const isBusy = sendStatus === 'sending' || sendStatus === 'streaming'
  const statusMessage =
    sendStatus === 'sending'
      ? 'Sending your message and starting the live response stream…'
      : sendStatus === 'streaming'
        ? 'Assistant is streaming a response… You can send another message when the reply arrives.'
  ...
  return (
    <div className="border-t border-gray-800 px-4 py-3 bg-gray-950" aria-busy={isBusy}>
      <div className="flex gap-2 items-end max-w-4xl mx-auto">
        ...
        <button
          ...
          aria-disabled={isBusy || !value.trim()}
        >
          {isBusy ? (
            <>
              <span
                ...
                aria-hidden="true"
              />
              <span>{sendStatus === 'streaming' ? 'Streaming…' : 'Sending…'}</span>
            </>
          ) : (
            'Send'
          )}
