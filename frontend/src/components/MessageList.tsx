...
export function MessageList({
  ...
  const isConnecting = sendStatus === 'sending'

  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [isBusy, isLoadingHistory, messages])
    ...
    <div className="flex-1 overflow-y-auto px-4 py-4" aria-busy={isBusy}>
      {isLoadingHistory ? (
        ...
        <>
          ...
          {/* Connection indicator before the assistant placeholder is rendered */}
          {isConnecting && (
            <div className="flex justify-start mb-3">
              ...
              >
                <div className="flex gap-2 items-center min-h-5">
                  ...
                  <span className="text-sm text-gray-300">Connecting to assistant stream…</span>
                </div>
              </div>
            </div>
          )}
